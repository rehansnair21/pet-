...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_to_cart'])) {
    ...
    header('Location: ' . strtok($_SERVER["REQUEST_URI"], '?') . (isset($_SERVER['QUERY_STRING']) ? ('?' . $_SERVER['QUERY_STRING']) : '') . '#products');
    exit;
}

// handle remove from cart
if (isset($_GET['remove_from_cart'])) {
    $rid = intval($_GET['remove_from_cart']);
    if ($rid > 0 && isset($_SESSION['cart'][$rid])) {
        unset($_SESSION['cart'][$rid]);
    }
    header('Location: ' . strtok($_SERVER["REQUEST_URI"], '?') . '#cart');
    exit;
}
